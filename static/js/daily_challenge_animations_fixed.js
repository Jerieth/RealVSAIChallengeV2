 ...
 * 
 * Fixed version with null element checks and simplified logic
 */
...
// Game logo animation for revealing images
function initGameLogoAnimation(containerSelector, callback) {
    console.log("Game logo animation initialized for: " + containerSelector);
    ...
    animElement.className = 'game-animation text-center';
    animElement.innerHTML = `
        <div class="py-5">
            <div class="logo-animation mb-3">
                <i class="fas fa-camera-retro text-primary" style="font-size: 3rem;"></i>
                <i class="fas fa-robot text-danger ms-2" style="font-size: 3rem;"></i>
            </div>
            <h3 class="mb-0">Revealing Image...</h3>
        </div>
    `;
...
// Submit daily challenge answer via AJAX
...
function submitDailyFinalAnswer(event) {
    if (event) {
        event.preventDefault();
    }
    
    // Disable all answer buttons to prevent multiple submissions
    document.querySelectorAll('.final-answer-btn').forEach(btn => {
        btn.disabled = true;
    });
    
    // Get the form data
    const form = document.getElementById('game-form-final');
    if (!form) {
        console.error('Final game form not found');
        return;
    }
    ...
    formData.append('answer', answer);
    formData.append("action", "submit_daily_final_answer");
    
    // Show loading state
    const feedbackElement = document.getElementById('final-answer-feedback');
    if (feedbackElement) {
        feedbackElement.style.display = 'block';
        feedbackElement.innerHTML = `
            <div class="alert alert-info" role="alert">
    ...
    .then(data => {
        ...
        // Game is always over after final round
        window.gameCompleted = true; // Set flag for navigation warning
        
        // Show redirect timer
        const timerElement = document.getElementById('final-redirect-timer');
        if (timerElement) {
            timerElement.style.display = 'block';
        }
            ...
            const countdownInterval = setInterval(() => {
                ...
                if (countdownElement) {
                    ...
                    if (countdown <= 0) {
                        clearInterval(countdownInterval);
                        // Navigate to game over or victory page
                        window.location.href = '/controllers/daily_victory.php';
                    }
                } else {
                    clearInterval(countdownInterval);
                }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.answer-btn').forEach(btn => {
        btn.addEventListener('click', submitDailyAnswer);
    });
    
    // Add event listeners to final answer buttons
    document.querySelectorAll('.final-answer-btn').forEach(btn => {
        btn.addEventListener('click', submitDailyFinalAnswer);
    });
});